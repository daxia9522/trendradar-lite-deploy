...
class RSSParser:
    ...
    def parse(self, content: Union[str, bytes], feed_url: str = "") -> List[ParsedRSSItem]:
        ...
        Args:
            content: Feed 内容（XML / JSON，支持 str 或 bytes）
        ...
        # 优先传入原始 bytes，让解析器按 XML 声明/响应内容自行判断编码，
        # 避免 requests.response.text 因错误响应头导致乱码。
    ...
    def _parse_json_feed(self, content: Union[str, bytes], feed_url: str = "") -> List[ParsedRSSItem]:
        ...
        if isinstance(content, bytes):
            try:
                ...
                raise ValueError(f"JSON Feed 解码失败 ({feed_url}): {e}")
