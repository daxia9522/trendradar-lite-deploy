...
class SQLiteStorageMixin:
        ...
        try:
            ...
            cursor.execute("""
                SELECT i.id, i.title, i.feed_id, f.name as feed_name,
                       ...
                       i.first_crawl_time, i.last_crawl_time, i.crawl_count, i.guid
                FROM rss_items i
    ...
    def _detect_new_rss_items_impl(self, current_data: RSSData) -> Dict[str, List[RSSItem]]:
        ...
        优先比较同一订阅源内的 GUID；GUID 缺失时回退到 URL。
        """
        try:
            ...
            for feed_id, rss_list in historical_data.items.items():
                ...
                for item in rss_list:
                    ...
                    if first_time < current_time:
                        item_guid = str(getattr(item, "guid", "") or "").strip()
                        if item_guid:
                            historical_guids[feed_id].add(item_guid)
                        if item.url:
                            historical_urls[feed_id].add(item.url)

            has_historical_data = any(
                historical_guids.get(feed_id) or historical_urls.get(feed_id)
                for feed_id in set(historical_guids) | set(historical_urls)
            )
            if not has_historical_data:
                # 当天第一次抓取，没有“新增”概念
                return {}
            ...
            for feed_id, rss_list in current_data.items.items():
                guid_set = historical_guids.get(feed_id, set())
                url_set = historical_urls.get(feed_id, set())

                for item in rss_list:
                    item_guid = str(getattr(item, "guid", "") or "").strip()
                    if item_guid:
                        is_new = item_guid not in guid_set
                    else:
                        is_new = bool(item.url) and item.url not in url_set

                    if is_new:
                        new_items.setdefault(feed_id, []).append(item)
